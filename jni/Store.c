...
void throwNotExistingKeyException(JNIEnv* pEnv){
	...
	if(lClass != NULL){
		(*pEnv)->ThrowNew(pEnv, lClass, "Key does not exit");
	}
...
{
	jclass lClass = (*pEnv)->FindClass(pEnv, "com/example/exception/InvalidTypeException");
	if(lClass != NULL){
...
void throwStoreFullException(JNIEnv* pEnv){
	jclass lClass = (*pEnv)->FindClass(pEnv, "com/example/exception/StoreFullException");
	if(lClass != NULL){
		(*pEnv)->ThrowNew(pEnv, lClass, "Store entries is full");
	}
	(*pEnv)->DeleteLocalRef(pEnv, lClass);
}
